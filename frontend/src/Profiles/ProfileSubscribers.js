...
function ProfileSubscribers({access, setAccess, refresh, setRefresh, setRequestUser, requestUser, setRefreshRequired}) {

  const { username } = useParams();
  const navigate = useNavigate();

  const [profiles, setProfiles] = useState([])
  const [nextPage, setNextPage] = useState("/api/profiles/"+ username +"/subscribers?page=1")
  const [firstPage, setFirstPage] = useState("/api/profiles/"+ username +"/subscribers?page=1")
